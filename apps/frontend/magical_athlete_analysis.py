...
@app.cell
def _(BOARD_DEFINITIONS, GameEngine, GameState, RacerState, random):
    # Create racers
    racers = [
        RacerState(idx=0, name="Banana", position=0),
        RacerState(idx=1, name="Centaur", position=0),
        RacerState(idx=2, name="Magician", position=0),
        RacerState(idx=3, name="Scoocher", position=0),
    ]

    # Initialize game state
    board = BOARD_DEFINITIONS["standard"]()
    initial_state = GameState(racers, board=board)

    # Create engine with fixed seed for reproducibility
    rng = random.Random(42)
    engine = GameEngine(initial_state, rng, verbose=True)

    # Store game history (turn snapshots)
    game_history = []
...
def _(math, mo):
    ...
    def render_racetrack(turn):
        ...
        """
    return render_racetrack, turn_slider
