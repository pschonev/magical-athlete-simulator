...
class AbilityCopyLead(Ability):
    ...
    def execute(
        ...
        for ab_name in target.abilities:
            ...
            if ab_name in me.active_abilities:
                ...
                if current_event_type in ab_instance.triggers:
                    engine.log_info(
                        f"{self.name}: Manually triggering new ability {ab_name} "
                        f"because it missed the event loop."
                    )
        ...
        return AbilityTriggeredEvent(
            responsible_racer_idx=owner_idx, source=self.name, phase=event.phase
        )
    ...
    def get_auto_selection_decision(
        ...
        return ctx.options[0]  # type: ignore
